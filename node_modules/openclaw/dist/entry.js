...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		import("./version-B7TNfFMY.js").then((n) => n.r).then(({ VERSION }) => {
			console.log(VERSION);
		}).catch((error) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-B9_KMUzE.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-B2OIJrIW.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
